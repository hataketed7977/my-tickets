...
public class AuthRepository {
    ...
    public void createSession(
        ...
        createSession(tokenHash, userId, expiresAt, null);
    }
    ...
    public CliSessionRow findSessionByOAuthState(String state) {
        var rows = jdbc.query(
                """
                SELECT s.id_hash, u.id, u.name, u.avatar_url
                FROM user_session s
                JOIN app_user u ON u.id = s.user_id
                WHERE s.oauth_state = ?
                  AND s.expires_at > ?
                """,
                (row, index) -> new CliSessionRow(
                        row.getString("id_hash"),
                        new ApiModels.AppUser(
                                row.getString("id"),
                                row.getString("name"),
    ...
    public void deleteSessionByTokenHash(String tokenHash) {
        jdbc.update("DELETE FROM user_session WHERE id_hash = ?", tokenHash);
    }

    public record CliSessionRow(
            String tokenHash,
